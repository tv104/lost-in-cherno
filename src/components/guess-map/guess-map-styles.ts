...
export const createStyles = (isExpanded: boolean): Record<string, ThemeUIStyleObject> => ({
  container: {
    ...
    right: 4,
    zIndex: "map",
    transition: `width ${TRANSITION_DURATION} ${EASING_FN}, height ${TRANSITION_DURATION} ${EASING_FN}`,
    ...
    "& .map": {
      ...
      transition: `transform ${TRANSITION_DURATION} ${EASING_FN}, filter ${TRANSITION_DURATION} ${EASING_FN}`,
